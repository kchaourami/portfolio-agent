"""
Role :
Calculer, pour chaque ticker du portefeuille, un score
...
# Seuils de la table de décision 
...
def _compute_momentum_score(row: pd.Series) -> tuple[int, list[str]]:
    ...
    #Calcule le momentum_score (0-100, 50 = neutre) à partir de return_20d et relative_perf_5d — chaque indicateur déplace le score de +/-25 points autour de 50, plafonné à [0, 100]. 
...
def _compute_macro_score(regime: MacroRegime, sector: str | None) -> int:
    ...
    #Ajustement doux selon le régime macro — 50 = neutre. 
...
def compute_decisions(repo: DuckDBRepository) -> list[TickerDecision]:
    ...
    #Point d'entrée pur calcul — calcule une décision structurée pour chaque ticker du portefeuille, à partir de mart_portfolio_value
...
def run_decision_engine(repo: DuckDBRepository) -> list[TickerDecision]:
    ...
    #Point d'entrée utilisé par l'orchestrateur (LangGraph) — calcule les décisions ET les persiste dans la table `decisions`. 
